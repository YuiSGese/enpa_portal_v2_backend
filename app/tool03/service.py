...
import tempfile
import ftplib
import logging # <<< DÒNG NÀY ĐÃ ĐƯỢC THÊM
...
# --- ジョブステータスストレージ (インメモリ) ---
# 構造: { job_id: {"status": str, "progress": int, "total": int, "results": Dict[str, Dict], ...} }
...
def calculate_font_size(text: str, font_path: str, box_width: int, box_height: int) -> int:
    ...
    try:
        ...
        while font_size <= max_font_size:
            ...
            if bbox is None: return max(1, font_size - 1) # getbbox が None を返す場合を処理
            width = bbox[2] - bbox[0]
            height = bbox[3] - bbox[1]

            if width > box_width or height > box_height:
                return max(1, font_size - 1) # 直前のサイズを返す (最小1)
            font_size += 1
        return max(1, font_size - 1) # max_font_size でも超えない場合はそれを返す
    except IOError:
        logging.error(f"フォントファイルを開けません: {font_path}") # <<< logger を logging に修正
        return 1 # デフォルトの最小サイズを返す
    except Exception as e:
        logging.error(f"フォント '{font_path}' のサイズ計算中にエラー: {e}") # <<< logger を logging に修正
        return 1 # デフォルトの最小サイズを返す
...
class FactoryRegistry:
    ...
    def register_factory(self, key: str, factory_cls: type):
        ...
        logging.debug(f"Factory 登録済み: {key} -> {factory_cls.__name__}") # <<< logger を logging に修正

    def get_factory(self, key: str) -> 'BaseImageFactory':
        """キーに基づいて Factory のインスタンスを取得します。"""
        logging.debug(f"キー '{key}' の Factory を検索中") # <<< logger を logging に修正
        ...
        if not factory_cls:
            ...
            logging.debug(f"キー '{key}' が見つかりません。基本キー '{base_key}' を試行します") # <<< logger を logging に修正
            factory_cls = self._factories.get(base_key)

            # 基本キーも見つからない場合
            if not factory_cls:
                logging.error(f"キー '{key}' と基本キー '{base_key}' の両方に Template Factory が存在しません") # <<< logger を logging に修正
                raise ValueError(f"Template Factory が存在しません: {key}")

        logging.debug(f"キー '{key}' に対して Factory クラス {factory_cls.__name__} を使用します") # <<< logger を logging に修正
        # 毎回新しいインスタンスを生成
        return factory_cls()
...
class BaseImageFactory:
    ...
    def get_template_path(self, template_key: str, has_mobile_data: bool) -> Path:
        ...
        if has_mobile_data and mobile_template_path.exists():
            logging.debug(f"モバイルテンプレートを使用: {mobile_template_path}") # <<< logger を logging に修正
            return mobile_template_path

        # それ以外の場合は通常テンプレートを使用 (存在必須)
        if not normal_template_path.exists():
            logging.error(f"基本テンプレートが存在しません: {normal_template_path}") # <<< logger を logging に修正
            raise FileNotFoundError(f"基本テンプレートが存在しません: {normal_template_path}")

        logging.debug(f"通常テンプレートを使用: {normal_template_path}") # <<< logger を logging に修正
    ...
    def _get_text_size(self, text: str, font: ImageFont.FreeTypeFont) -> tuple[int, int]:
        try:
            ...
            logging.error(f"'{text}' の _get_text_size でエラー: {e}") # <<< logger を logging に修正
            return 0, 0
    ...
    def _place_text(self, draw: ImageDraw, params: Dict[str, Any]):
        ...
        if box_width <= 0 or box_height <= 0:
            logging.warning(f"テキスト '{text}' のバウンディングボックスが無効です: ({x1},{y1})-({x2},{y2})") # <<< logger を logging に修正
            return

        font_size = calculate_font_size(text, font_path, box_width, box_height)
        if font_size <= 0:
             logging.warning(f"テキスト '{text}' の計算済みフォントサイズが0以下です (ボックス: ({x1},{y1})-({x2},{y2}))") # <<< logger を logging に修正
             return
        ...
        try:
            ...
            logging.error(f"テキスト '{text}' (フォント {font_path} サイズ {font_size}) の描画中にエラー: {e}", exc_info=True) # <<< logger を logging に修正
    ...
    def _calculate_discount_display(self, regular_price_str: Optional[str], sale_price_str: Optional[str], discount_type: Optional[str]) -> str:
        ...
        try:
            ...
            logging.warning(f"割引計算エラー ({regular_price_str}, {sale_price_str}, {discount_type}): {e}") # <<< logger を logging に修正
            return ""
    ...
    def _place_price_group(self, draw: ImageDraw, price_params: Dict, unit_params: Dict, suffix_params: Dict):
        ...
        try:
            ...
            logging.error(f"価格 '{price_text}' の _place_price_group でエラー: {e}", exc_info=True) # <<< logger を logging に修正
    ...
    def draw(self, row_data: Tool03ProductRowInput, template_key: str) -> Image.Image:
        ...
        try:
            ...
            if has_mobile_data and template_path.name.endswith("-2.jpg") and hasattr(self, '_draw_mobile_details'):
                self.height = 1370
                logging.debug(f"モバイルテンプレート {template_path.name} のため、一時的に高さを 1370 に設定") # <<< logger を logging に修正
            ...
            if has_mobile_data and hasattr(self, '_draw_mobile_details') and callable(getattr(self, '_draw_mobile_details')):
                 if template_path.name.endswith("-2.jpg"):
                     logging.debug(f"{template_key} の _draw_mobile_details を呼び出し") # <<< logger を logging に修正
                     self._draw_mobile_details(draw_obj, row_data)
                 else:
                     logging.warning(f"モバイルデータはありますが、{template_key} のモバイルテンプレートが見つからないため、モバイル詳細はスキップします。") # <<< logger を logging に修正

            return img

        except FileNotFoundError:
            logging.error(f"テンプレートキー '{template_key}' (モバイル: {has_mobile_data}) のテンプレートファイルが見つかりません") # <<< logger を logging に修正
            raise
        except Exception as e:
            logging.error(f"テンプレートキー '{template_key}' の画像描画中に不明なエラー: {e}", exc_info=True) # <<< logger を logging に修正
            raise
        finally:
    ...
    def _draw_mobile_details(self, draw: ImageDraw, row_data: Tool03ProductRowInput):
        logging.debug(f"{self.__class__.__name__} のデフォルト _draw_mobile_details を呼び出し") # <<< logger を logging に修正
...
# (FactoryTypeA, FactoryTypeB, FactoryTypeC などのクラスは変更なし,
# 修正済みの BaseImageFactory を継承していることを確認)
...
async def generate_images_background(job_id: str, product_rows: List[Tool03ProductRowInput]):
    ...
    logging.info(f"[Job {job_id}] {len(product_rows)} 件の画像の処理を開始します。") # <<< logger を logging に修正
    ...
    try:
        for index, row in enumerate(product_rows):
            logging.debug(f"[Job {job_id}] 画像 {index + 1}/{len(product_rows)} を処理中: {row.productCode}") # <<< logger を logging に修正
            row_id = row.id
            ...
            if job_id in job_tracker:
                 ...
                 logging.warning(f"[Job {job_id}] 行 {index+1} の開始前に Job がトラッカーに存在しません") # <<< logger を logging に修正
                 return
            ...
            logging.debug(f"[Job {job_id}] 行 {index+1} を処理中:") # <<< logger を logging に修正
            logging.debug(f"  - 受信テンプレート名: '{row.template}'") # <<< logger を logging に修正
            logging.debug(f"  - 計算された base_key: '{base_key}'") # <<< logger を logging に修正
            logging.debug(f"  - 最終 factory_key: '{factory_key}'") # <<< logger を logging に修正
            ...
            try:
                ...
                logging.error(f"[Job {job_id}] 画像 {index + 1} ({row.productCode}, テンプレート '{factory_key}') の処理エラー: {e}") # <<< logger を logging に修正
                current_result_dict["status"] = "Error"
                current_result_dict["message"] = str(e)
                error_count += 1
            except Exception as draw_error:
                logging.error(f"[Job {job_id}] 画像 {index + 1} ({row.productCode}, テンプレート '{factory_key}') の描画中に不明なエラー: {draw_error}", exc_info=True) # <<< logger を logging に修正
                current_result_dict["status"] = "Error"
                ...
                if job_id in job_tracker:
                    ...
                    logging.warning(f"[Job {job_id}] 行 {index+1} の処理完了時に Job がトラッカーに存在しません") # <<< logger を logging に修正

            await asyncio.sleep(0.01)

        if job_id in job_tracker:
            final_status = "Completed" if error_count == 0 else "Completed with errors"
            logging.info(f"[Job {job_id}] 処理完了。ステータス: {final_status}。エラー: {error_count}/{len(product_rows)}。") # <<< logger を logging に修正

    except Exception as e:
        final_status = "Failed"
        logging.error(f"[Job {job_id}] バックグラウンドタスクで重大なエラーが発生: {e}", exc_info=True) # <<< logger を logging に修正
        if job_id in job_tracker:
        ...
        if job_id in job_tracker:
            ...
            logging.info(f"[Job {job_id}] 処理時間: {end_time - start_time:.2f} 秒。") # <<< logger を logging に修正
...
def create_job_zip_archive(job_id: str) -> Optional[str]:
    ...
    if not job_dir.is_dir():
        logging.error(f"Job ディレクトリが存在しません: {job_dir}") # <<< logger を logging に修正
    ...
    try:
        ...
        logging.info(f"Zip ファイルの作成に成功: {zip_path}") # <<< logger を logging に修正
        return zip_path
    except Exception as e:
        logging.error(f"Job {job_id} の Zip ファイル作成中にエラー: {e}", exc_info=True) # <<< logger を logging に修正
        zip_file = f"{zip_output_path_base}.zip"
        if os.path.exists(zip_file):
            try: os.remove(zip_file)
            except OSError as remove_e: logging.error(f"エラーが発生した一時 Zip ファイルを削除できません: {zip_file}, エラー: {remove_e}") # <<< logger を logging に修正
...
async def regenerate_specific_images_background(job_id: str, modified_rows: List[Tool03ProductRowInput]):
    ...
    logging.info(f"[Job {job_id}] {len(modified_rows)} 件の画像の再生成/追加を開始します。") # <<< logger を logging に修正

    current_job_data = job_tracker.get(job_id)
    if not current_job_data:
        logging.error(f"[Job {job_id}] 画像再生成のための Job が見つかりません (ロジックエラー?)。") # <<< logger を logging に修正
        return

    job_dir = JOB_STORAGE_BASE_DIR / job_id
    if not job_dir.is_dir():
         logging.error(f"[Job {job_id}] Job ディレクトリが存在しません: {job_dir}") # <<< logger を logging に修正
    ...
    if new_rows_count > 0:
        ...
        logging.info(f"[Job {job_id}] {new_rows_count} 件の新規行を検出。total を {updated_total} に更新しました。") # <<< logger を logging に修正
    ...
    try:
        for index, row in enumerate(modified_rows):
            row_id = row.id
            logging.debug(f"[Job {job_id}] 画像 {index + 1}/{len(modified_rows)} を再生成/追加中 (Row ID: {row_id}, {row.productCode})") # <<< logger を logging に修正
            ...
            try:
                ...
                logging.error(f"[Job {job_id}] 画像の再生成/追加エラー ({row.productCode}, テンプレート '{factory_key}'): {e}") # <<< logger を logging に修正
                current_result_dict["status"] = "Error"
                current_result_dict["message"] = str(e)
            except Exception as draw_error:
                logging.error(f"[Job {job_id}] 画像の再生成/追加中に不明なエラー ({row.productCode}, テンプレート '{factory_key}'): {draw_error}", exc_info=True) # <<< logger を logging に修正
                current_result_dict["status"] = "Error"
                current_result_dict["message"] = "画像描画中に不明なエラーが発生しました。"
            finally:
                 if job_id in job_tracker:
                     ...
                     logging.warning(f"[Job {job_id}] 再生成 Row {row_id} の処理完了時に Job がトラッカーに存在しません") # <<< logger を logging に修正
        ...
        if job_id in job_tracker:
             ...
             if completed_count == current_total:
                 ...
                 logging.info(f"[Job {job_id}] 画像の再生成/追加完了。最終ステータス: {final_status}。進捗: {completed_count}/{current_total}。") # <<< logger を logging に修正
             else:
                  final_status = "Processing"
                  logging.debug(f"[Job {job_id}] ジョブはまだ処理中です。進捗: {completed_count}/{current_total}") # <<< logger を logging に修正
        else:
             final_status = "Failed"

    except Exception as e:
        final_status = "Failed"
        logging.error(f"[Job {job_id}] 画像の再生成/追加中に重大なエラーが発生: {e}", exc_info=True) # <<< logger を logging に修正
        if job_id in job_tracker:
...
def upload_job_images_to_ftp(job_id: str, target: str):
    ...
    if not config:
        logging.error(f"[Job {job_id}] ターゲット '{target}' の FTP 設定が見つかりません") # <<< logger を logging に修正
        if job_id in job_tracker:
    ...
    job_dir = JOB_STORAGE_BASE_DIR / job_id
    if not job_dir.is_dir():
        logging.error(f"[Job {job_id}] アップロード対象の Job ディレクトリが存在しません: {job_dir}") # <<< logger を logging に修正
        if job_id in job_tracker:
            ftp_status_key = f"ftpUploadStatus{target.capitalize()}"
            ftp_error_key = f"ftpUploadError{target.capitalize()}"
            job_tracker[job_id][ftp_status_key] = "failed"
    ...
    logging.info(f"[Job {job_id}] FTP ターゲット '{target}' (ホスト: {config['host']}) へのアップロードを開始します。") # <<< logger を logging に修正
    ...
    if job_id in job_tracker:
         ...
         logging.warning(f"[Job {job_id}] FTP アップロード開始時に Job がトラッカーに存在しません。") # <<< logger を logging に修正
         return
    ...
    try:
        ...
        logging.info(f"[Job {job_id}] FTP ディレクトリに移動中: {config['remote_dir']}") # <<< logger を logging に修正
        try:
             ftp.cwd(config['remote_dir'])
        except ftplib.error_perm as e:
             if "550" in str(e):
                  try:
                       logging.warning(f"[Job {job_id}] ディレクトリ {config['remote_dir']} が存在しません。作成を試みます...") # <<< logger を logging に修正
                       ...
                       for part in parts:
                           ...
                           try:
                               ftp.mkd(current_dir)
                               logging.debug(f"[Job {job_id}] FTP ディレクトリを作成しました: {current_dir}") # <<< logger を logging に修正
                           except ftplib.error_perm as mkd_e:
                               if "550" not in str(mkd_e):
                                   raise
                       ftp.cwd(config['remote_dir'])
                       logging.info(f"[Job {job_id}] ディレクトリを作成し、{config['remote_dir']} に移動しました") # <<< logger を logging に修正
                  except ftplib.all_errors as mkd_e:
                       upload_error_msg = f"FTP ディレクトリ '{config['remote_dir']}' の作成/移動に失敗しました: {mkd_e}"
                       logging.error(f"[Job {job_id}] {upload_error_msg}", exc_info=True) # <<< logger を logging に修正
                       raise
             else:
        ...
        if not image_files_to_upload:
             logging.warning(f"[Job {job_id}] {target} にアップロードする正常な画像がありません。") # <<< logger を logging に修正
        ...
        for filename in image_files_to_upload:
            ...
            try:
                with open(local_path, 'rb') as file:
                    ...
                    logging.info(f"[Job {job_id}] ファイルのアップロードに成功: {filename} -> {target}") # <<< logger を logging に修正
                    successful_uploads += 1
            except ftplib.all_errors as upload_e:
                 err_msg = f"ファイル {filename} のアップロードエラー: {upload_e}"
                 logging.error(f"[Job {job_id}] {err_msg}", exc_info=True) # <<< logger を logging に修正
                 upload_errors.append(err_msg)

        if successful_uploads == total_to_upload:
             upload_status = "success"
             logging.info(f"[Job {job_id}] アップロード完了。成功: {successful_uploads}/{total_to_upload} ファイル (ターゲット: {target})。") # <<< logger を logging に修正
        else:
             upload_status = "failed"
             ...
             logging.error(f"[Job {job_id}] {upload_error_msg}") # <<< logger を logging に修正
        ...
        if job_id in job_tracker:
            ...
            logging.info(f"[Job {job_id}] FTP ステータス '{target}' を '{upload_status}' に更新しました。") # <<< logger を logging に修正
...
async def cleanup_old_jobs():
     ...
     if jobs_to_delete:
          logging.info(f"{len(jobs_to_delete)} 件の古いジョブのクリーンアップを準備中。") # <<< logger を logging に修正
          for job_id in jobs_to_delete:
               logging.info(f"古いジョブをクリーンアップ中: {job_id}") # <<< logger を logging に修正
               try:
                    ...
                    logging.error(f"Job {job_id} のクリーンアップ中にエラー: {e}") # <<< logger を logging に修正

     await asyncio.sleep(600) # 10 分
...
#     logging.info("古いジョブのクリーンアップタスクを起動します...") # <<< logger を logging に修正
